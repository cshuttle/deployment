...
# Get the absolute directory where the script is located, so it can be run from any location.
SCRIPT_DIR=$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )

# --- Configuration ---
# The cluster name is derived from the 'name' field in the cluster template.
CLUSTER_NAME="Talos"
# The main template file for cluster definition, path is now relative to the script's location.
TEMPLATE_FILE="$SCRIPT_DIR/../clusters/staging/cluster.yaml"
...
deploy_or_redeploy() {
    ...
    echo "--> [1/3] Syncing cluster template '$TEMPLATE_FILE' with Omni..."
